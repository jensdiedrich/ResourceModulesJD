...
module vm_nic '.bicep/nested_networkInterface.bicep' = [for (nicConfiguration, index) in nicConfigurations: {
  ...
  params: {
    networkInterfaceName: '${nicConfiguration.nicPrefix}${name}${nicConfiguration.nicSuffix}'
...
resource vm 'Microsoft.Compute/virtualMachines@2022-11-01' = {
  ...
  properties: {
    ...
    storageProfile: {
      ...
      osDisk: {
        name: '${osDisk.osDiskPrefix}${name}${osDisk.osDiskSuffix}'
      ...
      dataDisks: [for (dataDisk, index) in dataDisks: {
        lun: index
        name: '${dataDisk.dataDiskPrefix}${name}-${padLeft((index + 1), 3, '0')}${dataDisk.dataDiskSuffix}'
    ...
    networkProfile: {
      networkInterfaces: [for (nicConfiguration, index) in nicConfigurations: {
        ...
        id: az.resourceId('Microsoft.Network/networkInterfaces', '${nicConfiguration.nicPrefix}${name}${nicConfiguration.nicSuffix}')
      }]
    }
    diagnosticsProfile: {
      bootDiagnostics: {
...
module vm_backup '../../recovery-services/vaults/protection-containers/protected-items/main.bicep' = if (!empty(backupVaultName)) {
